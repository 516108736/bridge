...
var (
	...
	tokenList = []erc20{
		...
		{
			//http://data.fixer.io/api/latest?access_key=f18589408b0e306389ab9016643e86be&symbols=MNT
			//http://data.fixer.io/api/latest?access_key=f18589408b0e306389ab9016643e86be&symbols=USD
			name:    "MIR",
...
func shuttle() {
	...
	for index, v := range tokenList {
		...
		DisplayLogs(index, v.address, logs)
...
func DisplayLogs(index int, contractAddress string, logs []types.Log) {
	burnCnt := 0
	...
	fmt.Printf("%s,%s,%d,%d,%s,%.2f,%s,%s,%d,%s,%.2f,%s,%s\n", tokenList[index].name, contractAddress, burnCnt+transferCnt, burnCnt, burnAmount, new(big.Float).Mul(new(big.Float).SetInt(burnAmount), new(big.Float).SetFloat64(tokenList[index].price)), burnMaxx, burnMaxxTxHash, transferCnt, transferAmount, new(big.Float).Mul(new(big.Float).SetInt(transferAmount), new(big.Float).SetFloat64(tokenList[index].price)), transferMaxx, transferMaxxTxHash)
}
...
var (
	...
	priceTokenToAddress = map[string]float64{
		"WETH":  1481.03,
		"WBTC":  47457.0000,
		"USDT":  1.01,
		"LINK":  26.520,
...
func chainBradge() {
	...
	for token, _ := range mpTokenToAddress {
		...
		fmt.Printf("%s,%s,%d,%d,%s,%.2f,%s,%s,%d,%s,%.2f,%s,%s\n", token, mpTokenToAddress[token], deposit.allCnt+execute.allCnt, deposit.allCnt, deposit.amount, new(big.Float).Mul(new(big.Float).SetInt(deposit.amount), new(big.Float).SetFloat64(priceTokenToAddress[token])), deposit.maxxAmount, deposit.maxxAmountHash, execute.allCnt, execute.amount, new(big.Float).Mul(new(big.Float).SetInt(execute.amount), new(big.Float).SetFloat64(priceTokenToAddress[token])), execute.maxxAmount, execute.maxxAmountHash)
	}
}
